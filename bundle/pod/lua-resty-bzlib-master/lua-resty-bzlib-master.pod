...
Whatever C<compress:append()> is returned, you must call C<compress:finish()> after C<bzlib:new()>
...
The bzip2 stream contains header and end-of-stream itself. The C<decompress:append()> will 
...
should call C<Decompress:finish()> manually to deallocate the memory.
...
This library requires C<LuaJIT> and C<libbz2.so> to be installed.

For CentOS users, you can install C<libbz2.so> by the following command:
