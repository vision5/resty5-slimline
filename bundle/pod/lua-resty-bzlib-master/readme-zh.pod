...
无论 C<compress:append()> 是否成功，结束时都必须调用 C<compress:finish()>
...
因此在 C<decompress:append()> 方法中，在出错和解压结束时会自动调用结束方法，不需要使用者手动结束。

手动中止压缩：

如果需要在解压过程中，即在 C<local text, finish, err = decompress:append()> 方法返回的 
C<text> 不为 C<nil> 或 C<finish> 不为 C<true> ，需要手动停止解压，结束操作，此时应手动调用 
C<decompress:finish()> 方法，让 bzlib 释放内存。
