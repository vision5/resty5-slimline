...
[!L<Build Status|https://travis-ci.org/zmartzone/lua-resty-openidc.svg?branch=master>](https://travis-ci.org/zmartzone/lua-resty-openidc)
L<<img width="184" height="96" align="right" src="http:E<sol>E<sol>openid.netE<sol>wordpress-contentE<sol>uploadsE<sol>2016E<sol>04E<sol>oid-l-certification-mark-l-rgb-150dpi-90mm@2x.png" alt="OpenID Certification">|https://openid.net/certification>


=head1 lua-resty-openidc


B<lua-resty-openidc> is a library for L<NGINX|http://nginx.org/> implementing the
L<OpenID Connect|http://openid.net/specs/openid-connect-core-1_0.html> B<Relying Party (RP)>
and/or the L<OAuth 2.0|https://tools.ietf.org/html/rfc6749> B<Resource Server (RS)> functionality.

When used as an OpenID Connect Relying Party it authenticates users against an OpenID Connect
Provider using L<OpenID Connect Discovery|http://openid.net/specs/openid-connect-discovery-1_0.html>
...
It maintains sessions for authenticated users by leveraging C<lua-resty-session> thus offering
a configurable choice between storing the session state in a client-side browser cookie or use
in of the server-side storage mechanisms C<shared-memory|memcache|redis>.
...
B<lua-resty-openidc> depends on the following packages:
...
L<NGINX|http://nginx.org/> and L<`ngx_devel_kit`|https://github.com/simpl/ngx_devel_kit>

=item *

L<Lua|http://www.lua.org/> or L<LuaJIT|http://luajit.org/luajit.html>
...
The dependencies above come automatically with L<OpenResty|http://openresty.org/>. You will need
...
L<`lua-resty-session`|https://github.com/bungle/lua-resty-session>
...
L<`lua-resty-jwt`|https://github.com/cdbattags/lua-resty-jwt>


=back

The C<lua-resty-jwt> dependency above is I<not> required when running as an OAuth 2.0 Resource Server (only) using remote
...
If you're using C<opm> execute the following:

     opm install zmartzone/lua-resty-openidc

If you're using C<luarocks> execute the following:

     luarocks install lua-resty-openidc

Otherwise copy C<openidc.lua> somewhere in your C<lua_package_path> under a directory named C<resty>.
If you are using L<OpenResty|http://openresty.org/>, the default location would be C</usr/local/openresty/lualib/resty>.
...
=head4 Community Support

For generic questions, see the Wiki pages with Frequently Asked Questions at:  
L<https:E<sol>E<sol>github.comE<sol>zmartzoneE<sol>lua-resty-openidcE<sol>wiki|https://github.com/zmartzone/lua-resty-openidc/wiki>  
Any questions/issues should go to issues tracker.


=head4 Commercial Services

For commercial Support contracts, Professional Services, Training and use-case specific support you can contact:  
L<sales@zmartzone.eu|mailto:sales@zmartzone.eu>  


=head2 Sample Configuration for Google+ Signin


Sample C<nginx.conf> configuration for authenticating users against Google+ Signin, protecting a reverse-proxied path.
...
=head2 Check authentication only
...
=head2 Check authentication only and deny unauthenticated access
...
=head2 Sample Configuration for OAuth 2.0 JWT Token Validation


Sample C<nginx.conf> configuration for verifying Bearer JWT Access Tokens against a pre-configured secret/key.
...
=head2 Sample Configuration for PingFederate OAuth 2.0


Sample C<nginx.conf> configuration for validating Bearer Access Tokens against a PingFederate OAuth 2.0 Authorization Server.
...
=head2 Sample Configuration for passing bearer OAuth 2.0 access tokens as cookie


Sample C<nginx.conf> configuration for validating Bearer Access Tokens passed as cookie against a ORY/Hydra Authorization Server.
...
L<luacov|https://keplerproject.github.io/luacov/> coverage while
testing use
...
you can contact L<ZmartZone IAM|https://www.zmartzone.eu> as described above in the L<Support> section.*
