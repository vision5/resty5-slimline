...
lua-resty-jwt - L<JWT|http://self-issued.info/docs/draft-jones-json-web-token-01.html> for ngx_lua and LuaJIT

[!L<Build Status|https://img.shields.io/travis/SkyLothar/lua-resty-jwt.svg?style=flat-square>](https://travis-ci.org/SkyLothar/lua-resty-jwt)

B<Attention :exclamation: the hmac lib used here is L<lua-resty-hmac|https://github.com/jkeys089/lua-resty-hmac>, not the one in luarocks.>
...
opm: C<opm get SkyLothar/lua-resty-jwt>

=item *

luarocks: C<luarocks install lua-resty-jwt>

=item *

Head to L<release page|https://github.com/SkyLothar/lua-resty-jwt/releases> and download C<tar.gz>
...
the L<LuaJIT 2.0|http://luajit.org/luajit.html>,
the L<lua-resty-hmac|https://github.com/jkeys089/lua-resty-hmac>,
and the L<lua-resty-string|https://github.com/openresty/lua-resty-string>,
...
you need to specify this library's path in ngx_lua's L<lua_package_path|https://github.com/openresty/lua-nginx-module#lua_package_path> directive. For example, C<lua_package_path "/path/to/lua-resty-jwt/lib/?.lua;;";>.
...
C<syntax: local jwt_token = jwt:sign(key, table_of_jwt)>

sign a table_of_jwt to a jwt_token.

The C<alg> argument specifies which hashing algorithm to use (C<HS256>, C<HS512>, C<RS256>).


=head3 sample of table_of_jwt
...
C<syntax: local jwt_obj = jwt:verify(key, jwt_token [, claim_spec [, ...]])>

verify a jwt_token and returns a jwt_obj table.  C<key> can be a pre-shared key (as a string), I<or> a function which takes a single parameter (the value of C<kid> from the header) and returns either the pre-shared key (as a string) for the C<kid> or C<nil> if the C<kid> lookup failed.  This call will fail if you try to specify a function for C<key> and there is no C<kid> existing in the header.

See L<Verification> for details on the format of C<claim_spec> parameters.
...
The C<alg> argument specifies which hashing algorithm to use for encrypting key (C<dir>).
The C<enc> argument specifies which hashing algorithm to use for encrypting payload (C<A128CBC-HS256>, C<A256CBC-HS512>)
...
Both the C<jwt:load> and C<jwt:verify_jwt_obj> functions take, as additional parameters, any number of optional C<claim_spec> parameters.  A C<claim_spec> is simply a lua table of claims and validators.  Each key in the C<claim_spec> table corresponds to a matching key in the payload, and the C<validator> is a function that will be called to determine if the claims are met.

The signature of a C<validator> function is:


    function(val, claim, jwt_json)

Where C<val> is the value of the claim from the C<jwt_obj> being tested (or nil if it doesn't exist in the object's payload), C<claim> is the name of the claim that is being verified, and C<jwt_json> is a json-serialized representation of the object that is being verified.  If the function has no need of the C<claim> or C<jwt_json>, parameters, they may be left off.

A C<validator> function returns either C<true> or C<false>.  Any C<validator> I<MAY> raise an error, and the validation will be treated as a failure, and the error that was raised will be put into the reason field of the resulting object.  If a C<validator> returns nothing (i.e. C<nil>), then the function is treated to have succeeded - under the assumption that it would have raised an error if it would have failed.

A special claim named C<__jwt> can be used such that if a C<validator> function exists for it, then the C<validator> will be called with a deep clone of the entire parsed jwt object as the value of C<val>.  This is so that you can write verifications for an entire object that may depend on one or more claims.

Multiple C<claim_spec> tables can be specified to the C<jwt:load> and C<jwt:verify_jwt_obj> - and they will be executed in order.  There is no guarantee of the execution order of individual C<validators> within a single C<claim_spec>.  If a C<claim_spec> fails, then any following C<claim_specs> will I<NOT> be executed.


=head3 sample C<claim_spec>
...
=head2 JWT Validators

A library of helpful C<validator> functions exists at C<resty.jwt-validators>.  You can use this library by including:

    local validators = require "resty.jwt-validators"

The following functions are currently defined in the validator library.  Those marked with "(opt)" means that the same function exists named C<< opt_<name> >> which takes the same parameters.  The "opt" version of the function will return C<true> if the key does not exist in the payload of the jwt_object being verified, while the "non-opt" version of the function will return false if the key does not exist in the payload of the jwt_object being verified.


=head4 C<validators.chain(...)>
Returns a validator that chains the given functions together, one after another - as long as they keep passing their checks.


=head4 C<validators.required(chain_function)>
Returns a validator that returns C<false> if a value doesn't exist.  If the value exists and a C<chain_function> is specified, then the value of C<chain_function(val, claim, jwt_json)> will be returned, otherwise, C<true> will be returned.  This allows for specifying that a value is both required I<and> it must match some additional check.


=head4 C<validators.require_one_of(claim_keys)>
Returns a validator which errors with a message if I<NONE> of the given claim keys exist.  It is expected that this function is used against a full jwt object.  The claim_keys must be a non-empty table of strings.


=head4 C<validators.check(check_val, check_function, name, check_type)> (opt)
Returns a validator that checks if the result of calling the given C<check_function> for the tested value and C<check_val> returns true.  The value of C<check_val> and C<check_function> cannot be nil.  The optional C<name> is used for error messages and defaults to "check_value".  The optional C<check_type> is used to make sure that the check type matches and defaults to C<type(check_val)>.  The first parameter passed to check_function will I<never> be nil.  If the C<check_function> raises an error, that will be appended to the error message.


=head4 C<validators.equals(check_val)> (opt)
Returns a validator that checks if a value exactly equals (using C<==>) the given check_value. The value of C<check_val> cannot be nil.


=head4 C<validators.matches(pattern)> (opt)
Returns a validator that checks if a value matches the given pattern (using C<string.match>).  The value of C<pattern> must be a string.


=head4 C<validators.any_of(check_values, check_function, name, check_type, table_type)> (opt)
Returns a validator which calls the given C<check_function> for each of the given C<check_values> and the tested value.  If any of these calls return C<true>, then this function returns C<true>.  The value of C<check_values> must be a non-empty table with all the same types, and the value of C<check_function> must not be C<nil>.  The optional C<name> is used for error messages and defaults to "check_values".  The optional C<check_type> is used to make sure that the check type matches and defaults to C<type(check_values[1])> - the table type.


=head4 C<validators.equals_any_of(check_values)> (opt)
Returns a validator that checks if a value exactly equals any of the given C<check_values>.


=head4 C<validators.matches_any_of(patterns)> (opt)
Returns a validator that checks if a value matches any of the given C<patterns>.


=head4 C<validators.contains_any_of(check_values,name)> (opt)
Returns a validator that checks if a value of expected type C<string> exists in any of the given C<check_values>.  The value of C<check_values>must be a non-empty table with all the same types.  The optional name is used for error messages and defaults to C<check_values>.


=head4 C<validators.greater_than(check_val)> (opt)
Returns a validator that checks how a value compares (numerically, using C<< > >>) to a given C<check_value>.  The value of C<check_val> cannot be C<nil> and must be a number.


=head4 C<validators.greater_than_or_equal(check_val)> (opt)
Returns a validator that checks how a value compares (numerically, using C<< >= >>) to a given C<check_value>.  The value of C<check_val> cannot be C<nil> and must be a number.


=head4 C<validators.less_than(check_val)> (opt)
Returns a validator that checks how a value compares (numerically, using C<< < >>) to a given C<check_value>.  The value of C<check_val> cannot be C<nil> and must be a number.


=head4 C<validators.less_than_or_equal(check_val)> (opt)
Returns a validator that checks how a value compares (numerically, using C<< <= >>) to a given C<check_value>.  The value of C<check_val> cannot be C<nil> and must be a number.


=head4 C<validators.is_not_before()> (opt)
...
=head4 C<validators.is_not_expired()> (opt)
...
=head4 C<validators.set_system_leeway(leeway)>
A function to set the leeway (in seconds) used for C<is_not_before> and C<is_not_expired>.  The default is to use C<0> seconds


=head4 C<validators.set_system_clock(clock)>
A function to set the system clock used for C<is_not_before> and C<is_not_expired>.  The default is to use C<ngx.now>


=head3 sample C<claim_spec> using validators
...
=head2 Legacy/Timeframe options

In order to support code which used previous versions of this library, as well as to simplify specifying timeframe-based C<claim_specs>, you may use in place of any single C<claim_spec> parameter a table of C<validation_options>.  The parameter should be expressed as a key/value table. Each key of the table should be picked from the following list.

When using legacy C<validation_options>, you I<MUST ONLY> specify these options.  That is, you cannot mix legacy C<validation_options> with other C<claim_spec> validators.  In order to achieve that, you must specify multiple options to the C<jwt:load>/C<jwt:verify_jwt_obj> functions.
...
C<lifetime_grace_period>: Define the leeway in seconds to account for clock skew between the server that generated the jwt and the server validating it. Value should be zero (C<0>) or a positive integer.
...
[Back to TOC](#table-of-contents)
When this validation option is specified, the process will ensure that the jwt contains at least one of the two C<nbf> or C<exp> claim and compare the current clock time against those boundaries. Would the jwt be deemed as expired or not valid yet, verification will fail.
...
C<require_nbf_claim>: Express if the C<nbf> claim is optional or not. Value should be a boolean.
...
[Back to TOC](#table-of-contents)
When this validation option is set to C<true> and no C<lifetime_grace_period> has been specified, a zero (C<0>) leeway is implied.
...
C<require_exp_claim>: Express if the C<exp> claim is optional or not. Value should be a boolean.
...
C<valid_issuers>: Whitelist the vetted issuers of the jwt. Value should be a array of strings.
...
[Back to TOC](#table-of-contents)
When this validation option is specified, the process will compare the jwt C<iss> claim against the list of valid issuers. Comparison is done in a case sensitive manner. Would the jwt issuer not be found in the whitelist, verification will fail.
...
=head3 sample of validation_options usage
...
L<JWT Auth With Query and Cookie|examples/README.md#jwt-auth-using-query-and-cookie>

=item *

L<JWT Auth With KID and Store Your Key in Redis|examples/README.md#jwt-auth-with-kid-and-store-keys-in-redis>
...
It is recommended to use the latest L<ngx_openresty bundle|http://openresty.org> directly.
